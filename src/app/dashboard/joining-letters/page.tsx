...
export default function JoiningLettersPage() {
  ...
  const filtered = joiningLetters.filter(
    (item) =>
  ...
  // Send joining letter via API
  async function handleSendJoiningLetter(candidate) {
    try {
      const res = await fetch("/api/send-joining-letter", {
        method: "POST",
      ...
      alert("Failed to send joining letter. Please check the console.");
    }
  }
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 px-4 py-8">
      ...
      <h1 className="text-4xl font-bold mb-8 text-center bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">Joining Letters</h1>
            ...
            filtered.map((item, idx) => (
              ...
              >
                ...
                <td className="px-6 py-4 flex gap-2 items-center">
                  <button
                    onClick={() => handleSendJoiningLetter(item)}
            ...
            <tr>
              <td colSpan={5} className="text-center py-12 text-gray-500 bg-gradient-to-r from-gray-50 to-blue-50">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-lg font-medium">No joining letters found.</span>
                </div>
              </td>
            </tr>
          )}
      ...
      {toast && (
        <div className="fixed bottom-8 right-8 bg-gradient-to-r from-green-500 to-emerald-500 text-white px-8 py-4 rounded-xl shadow-2xl z-50 border border-green-400 transform animate-bounce">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold">Joining letter sent!</span>
          </div>
        </div>
      )}
      </div>
